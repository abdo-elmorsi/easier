...
export const handleGetRequest = async (req, res) => {
  try {
    const { id, tower, payPercentage, search, for_select, for_settlement = false } = req.query;

    // Fetch a specific flat by ID
    if (id) {
      const flat = await prisma.Flat.findUnique({
        where: { id },
        include: { tower: { select: { name: true, id: true } } },
      });
      if (!flat) throw ErrorHandler.notFound("Flat not found");
      return res.status(200).json(flat);
    }

    // Ensure tower query parameter is provided
    if (!tower) throw ErrorHandler.badRequest("Tower query is required to retrieve flats.");
    ...
    const whereClause = {
      tower_id: tower, // Tower ID is mandatory
    ...
    if (for_settlement) {
      ...
      // Fetch all settlements for the current month in a single query
      const settlements = await prisma.Settlement.findMany({
        where: {
          tower_id: tower,
          created_at: { gte: monthStart, lte: monthEnd },
        },
        select: { flat_id: true }, // Only fetch the flat_id to minimize data
      });


      // Create a Set of flat IDs that have settlements
      const settledFlatIds = new Set(settlements.map((settlement) => settlement.flat_id));

      // Filter flats that are not in the settledFlatIds set
      const flatsWithoutSettlement = flats.filter((flat) => !settledFlatIds.has(flat.id));

      return res.status(200).json(flatsWithoutSettlement);
    }

    // Return all matching flats
