...
export const handleGetRequest = async (req, res) => {
  try {
    const { id, tower, payPercentage, search, for_monthly_settlement = false } = req.query;
    if (id) {
    ...
    if (for_monthly_settlement) {
      // Filter out flats that do not have a MonthlySettlement for the current month
      const flatsWithoutMonthlySettlement = [];
      for (const flat of flats) {
        ...
        if (!monthlySettlement) {
          flatsWithoutMonthlySettlement.push(flat);
        }
      }

      return res.status(200).json(flatsWithoutMonthlySettlement);
    }
