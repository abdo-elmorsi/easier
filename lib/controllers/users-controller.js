...
export const handlePostRequest = async (data, res) => {
  const { user_name, password, role = "admin", img, phone } = data;
  ...
  try {
    ...
    const newUser = await prisma.User.create({
      data: {
        user_name, password: hashedPassword, role, img: img || base64Image, phone
...
export const handlePutRequest = async (data, res) => {
  const { id, user_name, password, role, img, phone } = data;
  ...
  try {
    ...
    if (user_name && !isValidString(user_name)) return res.status(400).json({ message: "Username must be a valid non-empty string." });
