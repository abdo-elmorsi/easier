...
export const handleGetRequest = async (req, res) => {
  try {
    const { id, tower, month, search } = req.query;
    ...
    if (id) {
      ...
      if (!estimatedExpenses) throw new Error("EstimatedExpenses not found");

      return res.status(200).json(estimatedExpenses);
    }

    if (!tower) throw new Error("Tower query is required to retrieve estimated expenses.");
    ...
    if (!items.length) throw new Error("No estimated expenses found for the given month");
...
export const handlePostRequest = async (req, res) => {
  ...
  try {
    ...
    if (existingRecord) throw new Error("EstimatedExpenses for the current month already exists for this tower.");
...
export const handlePutRequest = async (req, res) => {
  ...
  try {
    ...
    if (existingMonthlySettlement) throw new Error("Cannot update EstimatedExpenses, MonthlySettlement exists for this tower in the current month.");
